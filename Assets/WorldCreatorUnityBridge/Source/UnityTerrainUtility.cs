    ...
    public static GameObject ImportPrefab(BridgeSettings settings, string name, string folderName, string path, string assetsDir, bool natureShader, bool importLOD)
    {
      ...
      for (int i = 0; i < (importLOD ? 3 : 1); i++)
      {
        ...
        if (billboard)
        {
          ...
          if (settings.MaterialType == MaterialType.HDRP)
          {
            var shader = Shader.Find("Shader Graphs/WCBillboardShaderGraph");

            mat = new Material(shader);
            var meshRenderer = lodObj.AddComponent<MeshRenderer>();
            meshRenderer.material = mat;
            lodObj.AddComponent<MeshFilter>().sharedMesh = AssetDatabase.LoadAssetAtPath<Mesh>("Assets/WorldCreatorUnityBridge/Content/Meshes/plane.fbx");
            mat.SetTexture("BaseMap", billboardDiff);
            mat.SetTexture("NormalMap", billboardNormal);
            renderers.Add(new LOD(0.5f / (i + 1), new[] { meshRenderer }));
          }
          else
          {
        ...
        foreach (XmlNode subset in doc.SelectNodes($"/WorldCreator/Mesh/LOD{i}/Subset"))
        {
          ...
          if (!string.IsNullOrEmpty(meshIndexString) && int.TryParse(meshIndexString, out var meshIndex) && int.TryParse(submeshIndexString, out var submeshIndex))
          {
            ...
            if (settings.MaterialType == MaterialType.URP)
            {
              mat = new Material(Shader.Find("Universal Render Pipeline/Nature/SpeedTree8"));
            ...
            else if (settings.MaterialType == MaterialType.HDRP)
            {
              ...
              mat.SetFloat("_DoubleSidedNormalMode", 0);
              mat.SetInt("_DoubleSidedEnable", doubleSided ? 1 : 0);
              mat.EnableKeyword("_NORMALMAP");
              mat.SetTexture("_NormalMap", normalTexture);
              mat.SetFloat("_AlphaCutoffEnable", alphaTest ? 1 : 0);
            }
            else
            {
              if (natureShader)
            ...
            mat.SetTexture("_BaseMap", diffuseTexture);
            mat.SetTexture("_BaseColorMap", diffuseTexture);
